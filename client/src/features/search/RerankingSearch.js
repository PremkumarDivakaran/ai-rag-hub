...
function RerankingSearch() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data.success) {
        // The API returns 'results' as the final reranked results (after fusion)
        // Use 'results' for afterReranking, and check if beforeReranking exists
        setResults(data.results || []);
        setBeforeResults(data.beforeReranking || []); // May not be provided by API
        setAfterResults(data.results || data.afterReranking || []); // Use results as afterReranking
        setSearchInfo({
          count: data.count,
        ...
        enqueueSnackbar(`Found ${data.count} results with ${(data.fusionMethod || 'rrf').toUpperCase()} score fusion`, { variant: 'success' });
      }
    } catch (err) {
  ...
  return (
    <Box sx={{ maxWidth: 1400, margin: 'auto', padding: 3 }}>
      ...
      {searchInfo && (
        ...
        >
          <Typography variant="body2">
            <strong>Score Fusion Results:</strong> Found {searchInfo.count} test cases using {(searchInfo.fusionMethod || 'rrf').toUpperCase()} method
            <br />
