...
Status: In Progress`;
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.log('📊 STEP 6: Summarization (LLM API) - Always make API call');
...
- Return valid JSON only`;
      ...
      console.log('🤖 STEP 8: LLM Generation (LLM API with Pre-processed Context)');
      ...
      // Handle response.raw format (JSON wrapped in markdown code blocks)
      if (parsedAnalysis && typeof parsedAnalysis === 'object' && parsedAnalysis.raw) {
        let rawContent = parsedAnalysis.raw;
      ...
      // If parsing still failed, provide a default structure
      if (!parsedAnalysis || !parsedAnalysis.overallRating) {
        parsedAnalysis = {
